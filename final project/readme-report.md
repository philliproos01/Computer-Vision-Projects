...
The final submission contains the main code files used for the trained document dewarping model:

- `model.py`: Defines the PyTorch document dewarping architecture. It builds the pretrained `timm` EfficientNet encoder, U-Net-style decoder with skip connections, 2-channel flow/UV prediction head, and differentiable unwarping step using `torch.nn.functional.grid_sample`.
- `train.py`: Trains the model on the synthetic document dataset. It loads warped RGB inputs, flat ground-truth targets, UV maps, and document masks; computes geometry-first losses such as UV/grid loss, SSIM loss, masked reconstruction loss, edge loss, and smoothness loss; and saves the best model checkpoint.
- `evaluate.py`: Loads a trained checkpoint and evaluates it on the validation split. This script is used for quantitative reporting, including validation loss and SSIM-style metrics.
- `inference.py`: Runs the trained model on one selected warped document image. It saves the rectified/dewarped output, predicted UV map, flow visualization, and optional debug images that show intermediate pipeline behavior.
- `best_model.pth`: Actual trained model. The model in the main folder is later referenced as the "first" model or as timm_geometry_outputs_b3_384_geomstrong.
- An additional experimental model I created is also in its own folder under /experimental_advanced_model

## How To Run The Submission

The commands below assume you are running from inside the `final_submission/` folder. If you run them from the main project folder instead, remove the leading `..\` from the dataset path.
...
`model.py` is not meant to be run by itself. It contains the PyTorch model definition used by the other scripts. The training, evaluation, inference, and comparison scripts import it internally:

```python
from model import build_model
```

The model uses a pretrained `timm` EfficientNet encoder, U-Net-style decoder, 2-channel flow/grid prediction head, and `torch.nn.functional.grid_sample` for differentiable dewarping.

### Training With `train.py`

To retrain the main balanced model:

```powershell
python train.py `
  --data-dir ..\renders\synthetic_data_pitch_sweep `
  --epochs 100 `
  --batch-size 4 `
  --img-size 384 384 `
  ...
  --output-dir timm_geometry_outputs_b3_384_geomstrong `
  --amp
```

This saves a best checkpoint, latest checkpoint, preview image, loss curves, and training history in the chosen output directory. The submitted `best_model.pth` was copied from the best checkpoint produced by this training setup.

### Evaluating With `evaluate.py`

To compute validation metrics and generate a preview image:
...
This reports validation losses and SSIM-style metrics. It is used for quantitative model evaluation.

### Running Single-Image Inference

To run the trained model on one warped document image:

```powershell
python inference.py `
  --checkpoint best_model.pth `
  --data-dir ..\renders\synthetic_data_pitch_sweep `
  --input ..\renders\synthetic_data_pitch_sweep\rgb\YOUR_IMAGE_NAME.jpg `
  --output output_dewarped.png `
  --uv-output predicted_uv.png `
  --flow-output flow_magnitude.png `
...
- `output_dewarped.png`: the model's rectified output.
- `predicted_uv.png`: the predicted UV/grid visualization.
- `flow_magnitude.png`: the predicted displacement magnitude.
- `debug_output/debug_panel.png`: a full debug panel showing input, encoder features, decoder features, predicted grid, model output, ground truth, and UV-dewarp oracle.

### Recreating The Comparison Images

The side-by-side comparison images in this report were generated with `compare_uv_dewarp_vs_timm.py`. This script compares the learned model against the UV-dewarp oracle created from the provided UV maps.

To recreate comparison images for three samples:

```powershell
python compare_uv_dewarp_vs_timm.py `
  --checkpoint best_model.pth `
  --data-dir ..\renders\synthetic_data_pitch_sweep `
...
- warped input
- model dewarped output
- UV-dewarp oracle output
- flat ground truth
- model-vs-UV difference
- predicted grid lines
- target UV grid lines
- model edge map
- UV oracle edge map

The report images `geomstrong_comparison.png` and `textgeom_gridgrad_comparison.png` are examples of these comparison panels.

# Project Goal

This project builds a machine-learning based document dewarper for images of pages that are folded, crumpled, curved, or otherwise non-rigidly deformed. In this report I explore two model approaches, a simplified model and my attempt at building a more advanced model. Both models had strengths and weaknesses that the other model did not. Project was trained on my local computer using an RTX 2080.
...
# "Advanced" Model: `timm_geometry_outputs_b3_textgeom_gridgrad`
...
![TextGeom grid/gradient comparison](textgeom_gridgrad_comparison.png)
...
The most important next improvement would be to precompute cleaner inverse sampling-grid targets from the UV maps. Instead of generating a noisy inverse grid inside every batch, a preprocessing step could create stable target grids with better splatting, interpolation, confidence masks, and hole filling. This would make direct grid supervision and grid-gradient supervision more reliable. Additionally, the largest bottleneck for this project was honestly time and computing. I was limited to training multiple iterations of models on my local computer using an RTX 2080 which took usually a day per model to train, making this project one of the most time consuming. The batch number, picture resolution and number of epochs also had to be low in order to not let the model crash during training. If I were to do this project (or similar) in the future I would definitely utilize cloud GPU computing for faster and better computing capabilities.
...
- Explore a transformerbackbone for stronger long-range fold understanding.

Overall, the best current model for the project requirements is `timm_geometry_outputs_b3_384_geomstrong`, while `timm_geometry_outputs_b3_textgeom_gridgrad` has the most promising potential as a more "advanced" version for future work.
